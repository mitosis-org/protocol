...
import { ITestnetOracle } from '../../interfaces/hub/oracle/ITestnetOracle.sol';
import { ERC7201Utils } from '../../lib/ERC7201Utils.sol';
import { StdError } from '../../lib/StdError.sol';

contract TestnetOracle is ITestnetOracle, Ownable2StepUpgradeable, AccessControlUpgradeable {
  ...
  /// @custom:storage-location mitosis.storage.TestnetOracle
  struct TestnetOracleStorage {
  ...
  string private constant _NAMESPACE = 'mitosis.storage.TestnetOracle';
  bytes32 private immutable _slot = _NAMESPACE.storageSlot();

  function _getTestnetOracleStorage() internal view returns (TestnetOracleStorage storage $) {
    bytes32 slot = _slot;
  ...
  function getPrice(bytes32 id) external view returns (Price memory) {
    TestnetOracleStorage storage $ = _getTestnetOracleStorage();
  ...
  function getEOLVaultPriceConfig(bytes32 id) external view returns (EOLVaultPriceConfig memory) {
    TestnetOracleStorage storage $ = _getTestnetOracleStorage();
    require($.eolVaultPriceConfigs[id].eolVault != address(0), ITestnetOracle__InvalidPriceId(id));
  ...
  function _getStoredPrice(TestnetOracleStorage storage $, bytes32 id) internal view returns (Price memory) {
    require($.prices[id].updatedAt != 0, ITestnetOracle__InvalidPriceId(id));
    return $.prices[id];
  }

  function _getEOLVaultPrice(TestnetOracleStorage storage $, IEOLVault eolVault, bytes32 underlyingPriceId)
    internal
    view
