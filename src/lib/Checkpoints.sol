...
library Checkpoints {
   ...
   * library.
   */
  function push(Trace storage self, uint48 timestamp, uint208 balance)
    internal
    returns (uint208 lastBalance, uint208 currentBalance)
  {
   ...
   * there is none.
   */
  function lowerLookup(Trace storage self, uint48 timestamp) internal view returns (uint208 balance, uint48 position) {
    ...
    return (ckpt._balance, ckpt._timestamp);
  }
   ...
   * if there is none.
   */
  function upperLookup(Trace storage self, uint48 timestamp) internal view returns (uint208 balance, uint48 position) {
    uint256 len = self._checkpoints.length;
   ...
   */
  function latest(Trace storage self) internal view returns (uint208 balance, uint48 position) {
   ...
   */
  function latestCheckpoint(Trace storage self) internal view returns (bool exists, uint48 position, uint208 balance) {
    uint256 pos = self._checkpoints.length;
    if (pos == 0) {
      return (false, 0, 0);
    } else {
      Checkpoint memory ckpt = _unsafeAccess(self._checkpoints, pos - 1);
      return (true, ckpt._timestamp, ckpt._balance);
    }
  }
   ...
   */
  function _insert(Checkpoint[] storage self, uint48 timestamp, uint208 balance)
    private
    returns (uint208 lastBalance, uint208 currentBalance)
  {
    ...
    if (pos > 0) {
      ...
      if (last._timestamp == timestamp) _unsafeAccess(self, pos - 1)._balance = balance;
      else self.push(Checkpoint({ _timestamp: timestamp, _balance: balance }));
      return (last._balance, balance);
    } else {
      self.push(Checkpoint({ _timestamp: timestamp, _balance: balance }));
      return (0, balance);
    }
  }
