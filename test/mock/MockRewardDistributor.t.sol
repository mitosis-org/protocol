...
contract MockRewardDistributor is IRewardDistributor, BaseHandler {
  ...
  function batchTimestamps(address, uint256, uint256) external view returns (uint48[] memory result) { }

  function batchTimestampsLength(address, address) external pure returns (uint256) {
    return 0;
  }

  function claim(address, address, bytes calldata) external { }

  function claim(address, address, address, bytes calldata) external { }

  function claim(address, address, address, uint256, bytes calldata) external { }
