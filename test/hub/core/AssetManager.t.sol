...
import { MockEOLRewardTreasury } from '../../mock/MockEOLRewardTreasury.t.sol';
...
contract AssetManagerTest is Toolkit {
  ...
  MockEOLRewardTreasury _eolRewardTreasury;
  ...
  function setUp() public {
    ...
    _eolRewardTreasury = new MockEOLRewardTreasury();
  ...
  function test_settleExtraRewards() public {
    ...
    vm.startPrank(owner);
    _assetManager.setRewardTreasury(address(_eolRewardTreasury));
  ...
  function test_settleExtraRewards_EOLRewardTreasuryNotSet() public {
    MockERC20TWABSnapshots rewardToken = new MockERC20TWABSnapshots();
    rewardToken.initialize(address(_delegationRegistry), 'Reward', '$REWARD');

    vm.startPrank(owner);
    // _assetManager.setRewardTreasury(address(_eolRewardManager));
    _assetManager.setAssetPair(address(rewardToken), branchChainId, branchRewardTokenAddress);
    vm.stopPrank();

    vm.startPrank(address(_assetManagerEntrypoint));

    vm.expectRevert(_errEOLRewardTreasuryNotSet());
    _assetManager.settleExtraRewards(branchChainId, address(_eolVault), branchRewardTokenAddress, 100 ether);
  ...
  function test_setRewardTreasury() public {
    assertEq(_assetManager.rewardTreasury(), address(0));

    vm.prank(owner);
    _assetManager.setRewardTreasury(address(_eolRewardTreasury));

    assertEq(_assetManager.rewardTreasury(), address(_eolRewardTreasury));

    MockEOLRewardTreasury newEOLRewardTreasury = new MockEOLRewardTreasury();

    vm.prank(owner);
    _assetManager.setRewardTreasury(address(newEOLRewardTreasury));

    assertEq(_assetManager.rewardTreasury(), address(newEOLRewardTreasury));
  }

  function test_setRewardTreasury_Unauthorized() public {
    vm.expectRevert(_errOwnableUnauthorizedAccount(address(this)));
    _assetManager.setRewardTreasury(address(_eolRewardTreasury));
  }

  function test_setRewardTreasury_InvalidParameter() public {
    vm.startPrank(owner);

    vm.expectRevert(_errInvalidParameter('EOLRewardTreasury'));
    _assetManager.setRewardTreasury(address(0));
  ...
  function _errEOLRewardTreasuryNotSet() internal pure returns (bytes memory) {
    return abi.encodeWithSelector(AssetManagerStorageV1.AssetManagerStorageV1__EOLRewardTreasuryNotSet.selector);
  }
}
