...
import { GlobalAccessControlManager } from '../../src/auth/GlobalAccessControlManager.sol';
import { MitosisVault } from '../../src/branch/MitosisVault.sol';
import { IGlobalAccessControlManager } from '../../src/interfaces/auth/IGlobalAccessControlManager.sol';
...
contract MitosisVaultTest is Test {
  GlobalAccessControlManager _globalAccessControlManager;
  ...
  function setUp() public {
    ...
    GlobalAccessControlManager globalAccessControlManagerImpl = new GlobalAccessControlManager();
    _globalAccessControlManager = GlobalAccessControlManager(
      payable(
        address(
          new TransparentUpgradeableProxy(
            address(globalAccessControlManagerImpl),
            address(_proxyAdmin),
            abi.encodeCall(_globalAccessControlManager.initialize, (owner, owner))
          )
        )
      )
    );
    ...
    _mitosisVault = MitosisVault(
      payable(
        address(
          new TransparentUpgradeableProxy(
            address(mitosisVaultImpl),
            address(_proxyAdmin),
            abi.encodeCall(mitosisVaultImpl.initialize, (owner, address(_globalAccessControlManager)))
          )
        )
      )
    );

    // // // // // GlobalAccessControlManager GRANT // // // // //
    vm.startPrank(owner);

    _globalAccessControlManager.grantPauseManager(address(_mitosisVault), address(_mitosisVault));

    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.initializeAsset.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.initializeEOL.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.redeem.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.allocateEOL.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.deallocateEOL.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.fetchEOL.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.returnEOL.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.settleYield.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.settleLoss.selector, address(_mitosisVault)
    );
    _globalAccessControlManager.grantRoleManager(
      address(_mitosisVault), MitosisVault.settleExtraRewards.selector, address(_mitosisVault)
    );
    ...
    _globalAccessControlManager.grant(address(_mitosisVault), MitosisVault.resumeAssetDeposit.selector, owner);
    _globalAccessControlManager.grant(address(_mitosisVault), MitosisVault.haltDepositWithOptIn.selector, owner);
    _globalAccessControlManager.grant(address(_mitosisVault), MitosisVault.resumeAssetDepositWithOptIn.selector, owner);
    _globalAccessControlManager.grant(address(_mitosisVault), MitosisVault.haltFetchEOL.selector, owner);
    _globalAccessControlManager.grant(address(_mitosisVault), MitosisVault.resumeFetchEOL.selector, owner);
  ...
  function test_deposit() public {
    ...
    vm.prank(owner);
    _mitosisVault.resumeAssetDeposit(address(_token));
  ...
  function test_deposit_AssetHalted() public {
    ...
    vm.expectRevert(_errPaused(MitosisVault.deposit.selector));
    _mitosisVault.deposit(address(_token), user1, 100 ether);
  ...
  function test_depositWithOptIn() public {
    ...
    vm.prank(owner);
    _mitosisVault.resumeAssetDepositWithOptIn(address(_token));
  ...
  function test_depositWithOptIn_AssetHalted() public {
    ...
    vm.expectRevert(_errPaused(MitosisVault.depositWithOptIn.selector));
    _mitosisVault.depositWithOptIn(address(_token), user1, hubEOLVault, 100 ether);
  ...
  function test_fetchEOL_AssetHalted() public {
    ...
    vm.prank(owner);
    _mitosisVault.haltFetchEOL(hubEOLVault);

    vm.startPrank(address(_strategyExecutor));

    vm.expectRevert(_errPaused(MitosisVault.fetchEOL.selector));
    _mitosisVault.fetchEOL(hubEOLVault, 10 ether);
  ...
  function _errPaused(bytes4 sig) internal pure returns (bytes memory) {
    return abi.encodeWithSelector(IGlobalAccessControlManager.IGlobalAccessControlManager__Paused.selector, sig);
  }

  function _errNotPaused(bytes4 sig) internal pure returns (bytes memory) {
    return abi.encodeWithSelector(IGlobalAccessControlManager.IGlobalAccessControlManager__NotPaused.selector, sig);
  }
